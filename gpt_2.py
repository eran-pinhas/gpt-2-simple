...
def download_file_with_progress(url_base, sub_dir, model_name, file_name):
    ...
    with open(os.path.join(sub_dir, file_name), "wb") as f:
        ...
        with tqdm(ncols=100, desc="Fetching " + file_name, total=file_size, unit_scale=True) as pbar:
...
def finetune(
    ...
    def save():
        ...
        saver.save(sess, os.path.join(checkpoint_path, "model"), global_step=counter - 1)
    ...
    try:
        while True:
            ...
            if accumulate_gradients > 1:
                ...
                (_, v_loss, v_summary) = sess.run((opt_apply, loss, summary_loss), feed_dict={context: batch})
...
def cmd():
    ...
    parser.add_argument("--dataset", help="[finetune] Path to the source text.", nargs="?", default=None)
    parser.add_argument(
        "--steps",
